<template>
    <div class="search-page">
        <app-header :titleShow="false">
            <input slot="center" class="search-input center" type="text" v-model="keywords" placeholder="玫瑰"/>
            <span slot="btn" class="search-btn" @click="handleProdList(keywords)">搜索</span>
        </app-header>
            ...
            <mt-tab-container v-model="selected">
                <mt-tab-container-item id="1">
                    <div v-for="hot in hotSearchs" v-bind:key="hot.title"
                         ...
                         @click="handleProdList(hot.title)">{{ hot.title }}
                    </div>
                </mt-tab-container-item>
                <mt-tab-container-item id="2">
                    <div v-for="hot in hotSearchs" v-bind:key="hot.title"
                         v-bind:class="['hot-item', hot.highlight ? 'highlight' : '']"
                         @click="handleProdList(hot.title)">{{ hot.title }}1
                    </div>
                </mt-tab-container-item>
            </mt-tab-container>
        </div>
        ...
        <div class="history">
            <div class="history-item" v-for="item in historyArr" :key="item.goods_id"
                 @click="handleProdList(item.name)">{{item.name}}
            </div>
        </div>
